...
do
    nVID=0 #$(perl -nle'print $& while m{(?<=nVID )[0-9]+}g' ${sourceFolder}/*-${run_num}.vci)
    nLVD=0 #$(perl -nle'print $& while m{(?<=nLVD )[0-9]+}g' ${sourceFolder}/*-${run_num}.vci)
    nFDO=$(perl -nle'print $& while m{(?<=nFDO )[0-9]+}g' ${sourceFolder}/*-${run_num}.vci)
    nSSH=0 #$(perl -nle'print $& while m{(?<=nSSH )[0-9]+}g' ${sourceFolder}/*-${run_num}.vci)
    nVIP=0 #$(perl -nle'print $& while m{(?<=nVIP )[0-9]+}g' ${sourceFolder}/*-${run_num}.vci)
    ...
    do
        ...
        opp_scavetool export -f "(module=~*router1.ppp[$sliNum]* AND name=~*xPk*) OR (module=~*router1.ppp[$sliNum]* AND name=~*packetP*)"  -F CSV-S -o ${resultsFolder}/${subfolderName}/vectors/${experimentDescriptor}_${totalNum}_VID${nVID}_LVD${nLVD}_FDO${nFDO}_SSH${nSSH}_VIP${nVIP}_resAllocLink${sliNum}_vec.csv ${sourceFolder}/*-${run_num}.vec
    done
    ...
    opp_scavetool export -f "module=~*serverFDO*"  -F CSV-S -o ${resultsFolder}/${subfolderName}/vectors/${experimentDescriptor}_${totalNum}_VID${nVID}_LVD${nLVD}_FDO${nFDO}_SSH${nSSH}_VIP${nVIP}_serverFDO_vec.csv ${sourceFolder}/*-${run_num}.vec
    ...
    do
        ...
        opp_scavetool export -f "module=~*hostFDO[$cli_num]*"  -F CSV-S -o ${resultsFolder}/${subfolderName}/vectors/${experimentDescriptor}_${totalNum}_VID${nVID}_LVD${nLVD}_FDO${nFDO}_SSH${nSSH}_VIP${nVIP}_hostFDO${cli_num}_vec.csv ${sourceFolder}/*-${run_num}.vec
