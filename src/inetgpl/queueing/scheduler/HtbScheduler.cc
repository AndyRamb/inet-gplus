...
HtbScheduler::htbClass *HtbScheduler::createAndAddNewClass(cValueMap *oneClass, int queueId, int numRates)
{
    ...
    long long mbuffer = oneClass->get("mbuffer").intValue() * 1e9;
...
//        newClass->leaf.queueLevel = 0;
        newClass->leaf.queueId = oneClass->get("queueNum").intValue();
        newClass->leaf.priority = oneClass->get("priority").intValue();
...
void HtbScheduler::handleMessage(cMessage *message)
{
    ...
    if (message == classModeChangeEvent) {
        ...
        for (auto & cl : leafClasses) {
            ...
            if (cl->assignedRate != -1000){
                // Deactivate the class if application stopped
                if(newRate == -1000){
                    cl->assignedRate = newRate;
                    deactivateClass(cl, 0);
                    std::cout << " Deactivating "<< "Tokens: "<< cl->tokens << " Ctokens: " << cl->ctokens << endl;
                }
                else {
                     //Set burst and GBR
                    ...
                    cl->assignedRate = newRate;
                    cl->rates.pop_front();
                    std::cout << "The new rate for: "<<cl->name <<" is: "<<newRate <<endl;

                    //Set cburst and MBR           
                    cl->cburstSize = newCBurst;
                    cl->ceilingRate = cl->ceils.front();
                    cl->ceils.pop_front();
                    std::cout << "The new ceil for "<<cl->name <<" is: "<<newCeil <<endl;
                }
            }
        }
        for (auto & cl : innerClasses) {
            ...
            //if (newRate == 0){
                //std::cout << "Numbers of packets before:" << collections[cl->leaf.queueId]->getNumPackets() << endl;
                //collections[cl->leaf.queueId]->removeAllPackets();
                //std::cout << "Numbers of packets after:" << collections[cl->leaf.queueId]->getNumPackets() << endl;
                //collections[cl->leaf.queueId]->removeAllPackets();
                //std::cout << "Numbers of packets after 2:" << collections[cl->leaf.queueId]->getNumPackets() << endl;
            //}
...
// Inform the htb about a newly enqueued packet. Enqueueing is actually done in the.
